...
class FeedbackRecord(Base, UUIDMixin, TimestampMixin):
    ...
    3. text_content, raw_data, created_at_source
    4. processing_status, analysis_summary
    5. created_at, updated_at (from TimestampMixin)
    """
    ...
    analysis_summary: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="ML analysis results summary"
    )
    ...
    __table_args__ = (
        ...
        Index("idx_feedback_analysis_summary", "analysis_summary", postgresql_using="gin"),
    )
